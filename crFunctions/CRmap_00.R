...
CRmap00 <- function(mapBorder = "County", mapLayer = "inc_rt", tmp_df = tmp_df){
  ...
  if("inc_rt" %in% mapLayer){
    popup <- paste0("GEOID: ", df.polygon@data$NAME
                    ...
                    , sep = " ")
    pal_rev <- rev(brewer.pal(5, "RdYlBu"))
    pal <- colorNumeric(palette = pal_rev
    ...
    zMap_border <- zMap_border %>%
      ...
      addLegend(pal = pal, values = log1p(df.polygon@data$incidence_rt), position = "bottomleft", title = "Incidence Rate of Cases", labFormat = labelFormat(suffix = " logscale cases per 100000"))
  }
